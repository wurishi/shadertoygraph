...
float Hash( float n )
{
	...
	return texture(iChannel2, vec2(n*0.93236, n*.51323), -100.0).x;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if (w > 0.0)
	{
		...
		if (uv.y > .13)
		{
			col = texture(iChannel0, vec2((uv.x+forTime)*.1, .2+uv.y*.1+floor((uv.x+forTime))*.1335)).yyy * .1;
		...
		else
		{
			...
			col *= texture(iChannel1, tex).yzz * .3 * winLight;
	...
	}else
	{
		...
		col = texture(iChannel0, st).xyz *.7;
	}
