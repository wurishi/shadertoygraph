...
// See:
// * https://iquilezles.org/prods/index.htm#kinderpainter
...
vec3 render( in vec2 fragCoord )
{
    ...
    fpar00[0] = vec4( 1.2*sin( 6.2831*.33*iTime + 0.0 ), 0.0,  
                      1.8*sin( 6.2831*.39*iTime + 1.0 ), 1 );
    fpar00[1] = vec4( 1.5*sin( 6.2831*.31*iTime + 4.0 ), 
                      1.0*sin( 6.2831*.29*iTime + 1.9),  
                      1.8*sin( 6.2831*.29*iTime + 0.0 ), 1 );
    ...
    float an = .15*iTime - 6.2831*iMouse.x/iResolution.x;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 col0 = render( (fragCoord.xy+vec2(0.0,0.0) ) );
    vec3 col1 = render( (fragCoord.xy+vec2(0.5,0.0) ) );
    vec3 col2 = render( (fragCoord.xy+vec2(0.0,0.5) ) );
    vec3 col3 = render( (fragCoord.xy+vec2(0.5,0.5) ) );
