...
vec3 ImageColour(vec2 vPixelCoord)
{
	...
	
	{
		...
		float fSpiral = abs( 0.5 - fract(fLength * 0.2 + fAngle * 3.14159 * 2.0 * 0.05 - iTime));
...
{
    float t = (fragRayOri.z - 5.0) / fragRayDir.z;
	vec3 vHit = fragRayOri + fragRayDir * t;  
    ...
    
    {
        ...
        vec2 vCoord2d = vUV * iResolution.xy;
        mainImage( vCol, vCoord2d );
        vFragColor.rgb += vCol.rgb;
        vFragColor.a += 1.0;
    }
    
    
    vFragColor.rgb /= vFragColor.a;
    
    vFragColor = vFragColor * vFragColor;
    vFragColor *= exp2(-abs(t) * 0.05);
    vFragColor = sqrt(vFragColor);
}