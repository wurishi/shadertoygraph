...
float bg(vec3 p){
	R(p.xz,1.3*cos(max(iTime-2.66,0.0)));
	vec3 sp = p;
	sp.xz+=1.0;
	sp.xz = mod(sp.xz,1.6)-0.8;
	sp.y+=1.3+0.1*cos(iTime+p.x);
	...
	R(sp.yz,iTime*0.02);
	sp = mod(sp,0.34)-0.17;
	float c = sph(sp,0.10+0.01*cos(iTime*7.0+p.x+p.y+p.z));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	c=(l.y==0.0?vec3(0.3,0.13*abs(0.2+0.5*cos((p.y+iTime)*3.0+0.5*cos(p.x*12.0))),0.1)*c.xxx:c);
	c=(l.y==2.0?vec3(1.0,+0.6+0.2*abs(cos(p.y*66.6-iTime*6.66+cos(p.x*33.0))),0.0):c);
