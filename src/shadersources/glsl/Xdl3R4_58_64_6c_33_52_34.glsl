...
float calcAO( in vec3 pos, in vec3 nor )
{
    ...
    return clamp( 1.5*totao/5.0, 0.0, 1.0 );
}
...
{ 
    ...
    if( t<maxd )
    {
        ...
        col = col*ao;
		col += 0.15*(0.5+0.5*kr)*pow(texture( iChannel1, normalize(sref+ref) ).xyz, vec3(1.0) );

        col = mix(col,vec3(1.0,0.9,0.8),1.0-exp( -0.05*t*t ));
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for( int i=0; i<16; i++ )
	    freqs[i] = clamp( 1.9*pow( texture( iChannel0, vec2( 0.02 + 0.5*float(i)/16.0, 0.25 ) ).x, 3.0 ), 0.0, 1.0 );
