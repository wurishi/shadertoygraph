...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 v;
	vec3 p = 4.*vec3(uv,0.)+iTime*(.1,.7,1.2);
	float x = myfbm(p);
	//v = vec3(x);
	...
	v *= g;
	vec3 Ti = texture(iChannel0,.02*v.xy+fragCoord.xy / iResolution.xy).rgb*1.4-.2;
	vec3 Tf = texture(iChannel1,.02*v.xy+fragCoord.xy / iResolution.xy).rgb;
	vec3 T=Ti;
	// T = Ti+(1.-Ti)*Tf; 
	vec3 T1,T2;
