...
{
	vec2 q = 16.0*p;
    return 0.5 + 0.5* checkersFiltered( q );
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if( t<511.0 )
	{
        ...
        vec2  gradG = cmul( z, conj(dz) );
        //vec2  gradG = conj(cmul( dz, conj(z) ));
        //vec2  gradG = cmul( conj(dz), z );
        //gradG = vec2(dFdx(greenR),dFdy(greenR))*iResolution.y;
        
        float n = t/50.0;
        float sn = -log2(abs(greenR))/50.0;
	
        col = vec3( 0.6 + 0.4*dot(normalize(gradG),vec2(0.707)) );
        col *= vec3( grid( ph*1.0 ) );
