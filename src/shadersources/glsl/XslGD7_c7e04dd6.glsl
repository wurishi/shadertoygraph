...
int aff(int b)
{
	...
	if((abs(uv.x)<.5)&&(abs(uv.y)<.5))    // pixel is in bbox
	{
		...
		float t=PI/4.; 
#if EFFECT>0
		float dt = DELAY*floor(iTime/DELAY); // effect every DELAY seconds
...
{
    pos *= iResolution.xy;
    scale *= iResolution.xy;
	FragCoord=fragCoord;
    int c;
	
	int t = int(iTime*100.);   // decompose 100*timer in digits 
	for (int i=0; i<5; i++) {
	...
	if (c>0) 
		fragColor = vec4(0.5+0.5*sin(iTime),uv,1.0);     // draw set pixels
	//else if (c==0)
	//	fragColor = vec4(0.*uv,0.5-0.5*sin(iTime),1.0);  // draw digit background
	else
		fragColor = vec4(uv,0.5-0.5*sin(iTime),1.0);     // draw background
}