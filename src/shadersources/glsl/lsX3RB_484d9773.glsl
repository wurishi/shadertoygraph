...
{
	return vec2(p.x + cos(a) * p.y * cos(a), p.x * cos(a) * p.y / cos(a));
}

// 1D random numbers
float rand(float n)
{
    return fract(sin(n) * 4.5453123);
}

// 2D random numbers
vec2 rand2(in vec2 p)
{
	return fract(vec2(sin(p.x * 1.32 + p.y * 15.077), cos(p.x * 1.32 + p.y * 40.077)));
}
...
vec2 voronoi(in vec2 x)
{
	...
	for(int j = -1; j <= 1; j ++)
	{
		...
		{
			vec2 b = vec2(i, j);
			vec2 r = vec2(b) * f * rand2(p + b)*0.023;
...
float scene(vec3 p)
{
	...
	return 0.877 - abs(p.x) + (ns.y + ns.x) * 2.5 + (cos(p.x * 5.5) + sin(p.y / 1.995)* sin(p.y + 100.5)) * 0.3;
}
...
vec3 shade(vec3 ray, vec3 dir, vec3 nml, float dist)
{
	...
	col += exp(-dist - 0.00999 * noise1(iTime * 2.0) * 0.5);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	dir.yz = rotate(dir.yz, cos(iTime * 0.7) * 70.5 * 23.995)* tan (iTime)*0.23;
	dir.xz = rotate(dir.xz, cos(iTime * 0.2) * 23.995);
	...
	if(dist < 1.0)
	{
		...
		{
			nml = normal(ray);
			col += 0.02 * shade(ray, ref, nml, dist);
		}
	}
