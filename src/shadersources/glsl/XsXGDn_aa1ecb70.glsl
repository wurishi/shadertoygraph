...
vec4 imageFunction(vec2 pos)
{
	...
	pos = pos * (1.5 + sin(iTime*0.05+origPos.x)*0.35+p1*0.2);
	pos = array(pos);
	vec4 rot = sinf(x_y_ang_dist(rotate(mirror(pos), iTime*0.05+p0*0.5)));	
	pos = rotate(pos, rot+origPos.y);
	return x(rotate(julia(pos), iTime*0.2+p2*0.0));
}
