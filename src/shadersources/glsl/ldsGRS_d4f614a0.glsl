...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if (material == 1) materialDiffuse = (texture(iChannel3, rayPos.zx).rgb / 2.0 + 0.5) * vec3(1.0, 0.75, 0.6) * 0.5;
	if (material == 2) materialDiffuse = vec3(1.0, 0.7, 0.6) * texture(iChannel1, vec2(polarXY.y * 2.0 / PI, rayPos.z + polarXY.x)).rgb;//vec3(0.0, 1.0, 0.0);
	if (material == 3) materialDiffuse = vec3(0.3, 0.5, 1.0) * texture(iChannel2, vec2(rayPos.x, rayPos.y + rayPos.z)).rgb;
