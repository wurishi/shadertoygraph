...
vec2 monster( vec3 p )
{
	p.x += 1.4*sin( 0.55*iTime + p.y );
	p.z += 1.4*sin( 0.75*iTime + p.y );
	
	p.x += sin( 3.2*p.y + 0.0*iTime );
    p.z += sin( 2.0*p.x  + 3.0*p.y - 0.0*iTime );

	p.xz *= 3.0-0.25*(p.y+0.55);//1.5 - 1.0*sin(0.2*iTime);

	p.y -= 0.4 + 5.0*sin(0.2*iTime);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if( tmat.y>0.5 )
    {
        ...
        if( tmat.y<1.5 )
		{
            ...
            vec3 mcol = texture( iChannel0, 0.5 + 0.5*nor.xy*vec2(-1.0,-1.0) + 0.5*pos.xy ).xyz;
            col *= 1.5*mcol;
