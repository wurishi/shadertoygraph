...
/* 
    (min
      (blend [-0.923,-0.359,-0.837,0.359] noise ridged-bw)
      (+ bwnoise ridged-bw))
*/
...
vec4 imageFunction(vec2 pos)
{
	...
	//return ridged_bw(rotateAndMove(pos, iTime*0.01),iTime*0.2);
	return 
		minf(
			ridged_bw(rotate(pos, p3*0.3),p2*2.0-iTime*0.2), 
			add(bw_noise(rotate(pos, p1*0.2), p1*2.0), ridged_bw(pos, p0*5.0)+iTime*0.1));
}
